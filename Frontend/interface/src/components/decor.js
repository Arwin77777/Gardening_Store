...
import dc from './images/minwater.jpeg';
const Decor = ()=>
{
  var a = "";
  ...
  axios.get('http://localhost:8181/api/v1/auth/getdecor')
  .then(response=>{
  ...
  .catch(error=>
    ...
    <div className='card-1'>
      <h3 className='card-header'>Garden Decors</h3>  
    <Row xs={1} md={4} className="g-4">
      {events.map(ev => (
        <Col onClick={()=>handleDetails(ev)} key={ev.product_id}>
          {/* <Link to='/details'> */}
          <Card style={{cursor:'pointer'}}>
            <Card.Img style={{width:'100%',height:'190px'}} variant="top" src={dc} />
            <Card.Body>
